...
<body>
    ...
    //https://www.youtube.com/watch?v=OMZa3bQZU44&list=PL-h5aNeRKouEaGrQj6EXaqZsagEphQboI&index=17
    if(isset($_POST['insert-categ'])){
        $category_title = $_POST['categ-title'];

        //Select Data from Database
        $select_query = "SELECT * from accessocategories WHERE category_title='$category_title'";
        ...
        if ($select_query = "SELECT * from categories WHERE category_title='$category_title'"){
            $result_select = mysqli_query($link, $select_query);
            $number = mysqli_num_rows($result_select);
                if ($number>0){
                    echo "<script>alert('Oops! This category already exists.')</script>";
        ...
        else if ($category_title=='' or $category_image==''){
            echo "<script>alert('Please fill all the details.')</script>";
            exit();
        } 
        else {
    ...
    ?>
        ...
        <div class="add-details-container">
            ...
            <div class="details-container">
                <form action="" method="post" class="ins-categ">
                    <label for="p-title">Category Title:</label>
                    <input type="text" id="p-title" name="categ-title">
                    <br>
